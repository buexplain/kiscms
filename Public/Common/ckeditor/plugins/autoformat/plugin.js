(function()
{
	CKEDITOR.plugins.add('autoformat',{
		init:function(editor){
			...
			editor.ui.addButton('Autoformat',{label:"清除格式",command:'autoformat',icon:CKEDITOR.getUrl( this.path + 'images/autoformat.gif' )});
		}
	});
})();
...
function FormatText(editor) {
   ...
   if (myeditor.mode=="wysiwyg"){
        ...
        if (!isPart) {
            ...
            var pres = tmpDiv.getElementsByTagName("PRE");
            ...
            if(temppre != null && temppre.length > 0) {
                for (var j = 0; j < temppre.length; j++) {
                    html = html.replace("#FormatPreID_" + j + "#", "<pre>" + temppre[j] + "</pre>");
                }
            }
